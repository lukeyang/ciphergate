...
CUSTOMER_MIN_INSTANCES="${CUSTOMER_MIN_INSTANCES:-1}"
CUSTOMER_MAX_INSTANCES="${CUSTOMER_MAX_INSTANCES:-1}"
CUSTOMER_CONCURRENCY="${CUSTOMER_CONCURRENCY:-1}"
...
gcloud run deploy "${CUSTOMER_SERVICE_NAME}" \
  ...
  --cpu 1 \
  --memory 2Gi \
  --min-instances "${CUSTOMER_MIN_INSTANCES}" \
  --max-instances "${CUSTOMER_MAX_INSTANCES}" \
  --concurrency "${CUSTOMER_CONCURRENCY}" \
